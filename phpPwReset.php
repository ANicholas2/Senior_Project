...
if($_SERVER['REQUEST_METHOD'] == "POST") {
	...
	if(empty($username)) {
		header("Location: pwReset.php?error=Username is required!");
	...
	$query = $db->prepare("UPDATE User SET pass=? WHERE uName= ?");
	$query->bind_param('ss', $pw, $username);
	
	if($query->execute()){
	} else {
		echo mysqli_error($db);
	}
	$query1 = $db->prepare("SELECT fName, lName, uName, gender, email, uID, position, imagePath FROM User WHERE pass=? AND uName= ?");
	$query1->bind_param('ss', $pw, $username);
